 ...
 * pre-filled, so the 90% WhatsApp-fast flow gets one-tap structure (Capture
 * Rail 1.1). Deterministic + offline (no model). Accept → book a card; ignore →
 * the text sends as a plain bubble. Proportional gating (the plan's discipline):
 * we only suggest the cheap-to-fix, low-fraud captures — attendance & delivery.
 * Money is never auto-suggested; it goes through an explicit slash-command.
 *
