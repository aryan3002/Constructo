 ...
 * (Capture Rail 1.1). A power user types `/del cement 50 bori ABC` faster than
 * the WhatsApp sentence it replaces; we parse it 100% client-side and post via
 * the existing `capture_type`/`fields` fast path (Phase 0.1 → confidence 1.0, no
 * model call). The canonical `capture_type` strings here are the aliases the
 * backend's `_CAPTURE_TYPE_ALIASES` already understands.
 *
