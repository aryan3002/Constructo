...
/** One row in the woven Home Room feed — a bubble, a curated update card, or an
 *  actionable decision card. Sorted oldest→newest (the thread scrolls to end). */
export type HomeRoomItem =
  | { kind: 'bubble'; key: string; at: number; message: ChatMessage }
...
/**
 * Weave chat messages, published updates, and pending decisions into one
 * chronological feed.
 *
 * - `decision_needed` updates are dropped: the pending Decision they describe
 *   gets its own actionable DecisionCard, so keeping the update too would
 *   double-surface the same ask.
 * - Only `pending` decisions render (an answered one is history, not an ask).
 * - The sort is stable (insertion-order tiebreak) so equal timestamps keep a
 *   predictable order across refetches.
 */
export function mergeHomeRoom(
  messages: ChatMessage[],
  updates: Update[],
  decisions: HomeownerDecision[],
): HomeRoomItem[] {
  const items: HomeRoomItem[] = []

  for (const m of messages) {
    items.push({ kind: 'bubble', key: `m:${m.id}`, at: epoch(m.created_at), message: m })
  }
  for (const u of updates) {
    if (u.type === 'decision_needed') continue // superseded by its DecisionCard
    items.push({ kind: 'update', key: `u:${u.id}`, at: epoch(u.published_at), update: u })
  }
  for (const d of decisions) {
    if (d.state !== 'pending') continue
    items.push({ kind: 'decision', key: `d:${d.id}`, at: epoch(d.created_at), decision: d })
  }
