/**
 * dp — the full design brief (Calm Cockpit). For each area, the AI-drafted
 * theme directions (palette + materials + rationale) with the owner/architect's
 * commit actions (approve / adjust / reject). The "designer's detailed brief".
 */
...
export default function DesignBrief() {
  ...
  const q = useQuery({
    queryKey: ['owner', 'design', 'brief', id],
    queryFn: async () => {
  ...
  const decide = useMutation({
    ...
    onSuccess: () => void qc.invalidateQueries({ queryKey: ['owner', 'design', 'brief', id] }),
  })
  ...
  return (
    <Pad>
      <SubHeader title="Design brief" sub="AI-drafted directions · you commit" onBack={() => router.replace('/(contractor)/owner/design')} />
      ...
      {totalThemes === 0 ? (
        ...
        groups.map(({ area, themes }) => (
          <AreaBlock
            ...
            onDecide={(themeId, action) => decide.mutate({ themeId, action })}
          />
        ))
      )}
    </Pad>
    ...
    <Card>
      ...
      {t.palette?.length ? (
        <>
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: SPACE.xs, marginTop: 6 }}>
            {t.palette.map((p, i) => <Chip key={i} label={p} />)}
          </View>
        </>
      ) : null}
      ...
      {t.materials?.length ? (
        <>
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: SPACE.xs, marginTop: 6 }}>
            {t.materials.map((m, i) => <Chip key={i} label={m} />)}
          </View>
        </>
      ) : null}
      ...
      {t.status === 'suggested' ? (
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: SPACE.md }}>
          ...
          <Small style={{ color: theme.colors[tone] }}>{cap(t.status)} by owner</Small>
        </View>
      )}
    </Card>
  )
