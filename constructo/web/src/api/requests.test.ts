 ...
 * (`app/requests/router.py::list_requests` — the owner-scoped twin of
 * `app/homeowner/router.py::list_requests`, which 403s an owner token),
 * including the optional `?site_id=` filter, and that the response is
 * returned as-is (typed `RequestOut[]`).
 */
...
describe('requestsApi.list', () => {
  it('GETs /api/v1/requests and returns the RequestOut[] as-is', async () => {
    ...
    expect(url).not.toContain('/homeowner/requests')
