/**
 * slash.ts — unit tests (web Phase B). Mirrors the mobile parser test
 * (`constructo/mobile/src/capture/slash.test.ts`) so the two stay in lock-step,
 * plus a check for the web-only SLASH_MENU display metadata.
 */
import { describe, it, expect } from 'vitest'
import { isSlash, parseSlash, SLASH_MENU, type ParsedCapture } from './slash'
...
describe('SLASH_MENU', () => {
  it('exposes the four commands with label + usage for the menu', () => {
    expect(SLASH_MENU.map((c) => c.cmd)).toEqual(['att', 'del', 'pay', 'inv'])
    for (const item of SLASH_MENU) {
      expect(item.label).toBeTruthy()
      expect(item.usage.startsWith('/')).toBe(true)
    }
  })
})
